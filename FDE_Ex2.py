# -*- coding: utf-8 -*-
"""
Created on Mon Nov 28 16:41:03 2022
...
def g(x):
    ...
    2x^{3} + x^{2} + x + 1
    '''
    return x**0.5 * ( 6*x*((x**2) / 6 + ((math.gamma(3.5) + 2 * math.gamma(2.5)) / (math.gamma(3.5) * math.gamma(2.5)))) - 2*((x**2) / 2 + ((math.gamma(2.5) + 2 * math.gamma(1.5)) / (math.gamma(2.5) * math.gamma(1.5)))))
...
def f(x, a_value, n=3):
    ...
    for idx, xx in enumerate(x):
        ...
        for i in range(n+1):
            total_value +=  (2 / xx) * a_value[i] * d(xx, i, 0.5) 
          
        for i in range(n+1):
            total_value += (xx ** 0.5) * a_value[i] * xx **i 


        g_result = g(xx)

        er = (total_value - g_result) ** 2 + lambda_v * (a_value[0] - 0) ** 2 + lambda_v * (a_value[0] - 0) ** 2 
        error += er
